...
def direct_answer(message, info):
    ...
    if any(word in text for word in ('اعلان', 'ترويج')):
        ...
        if package != 'vip':
            return 'إنشاء إعلان المؤسسة متاح في باقة VIP. افتح «الإعدادات ← الباقات» لمراجعة الباقات أو طلب الترقية.'
        return ('لإنشاء إعلان: افتح «الإعدادات ← الباقات ← إدارة إعلاني»، ثم اكتب عنوان الإعلان، نصه، ووسيلة التواصل، وبعدها اضغط «تشغيل الإعلان».\n'
