...
function supportActionLabel(category){if(category.includes('أجهزة'))return 'فتح أجهزة المؤسسة';if(category.includes('اشتراك'))return 'فتح الباقة والخدمات';if(category.includes('شات'))return 'فتح إعدادات الشات';if(category.includes('مواعيد'))return 'فتح إعدادات المواعيد';return 'فتح حساب المستخدم'}
...
async function loadSupportAppointments(organizationId){let r=await fetch('/owner/api/organizations/'+organizationId+'/appointments',{headers:hdr()}),d=await r.json();if(!r.ok){alert(d.error||'تعذر تحميل المواعيد');return}showSecurityPanel('supportAppointmentsPanel');let box=document.getElementById('supportAppointments');box.innerHTML=d.map(x=>`<div class="event"><b>${esc(x.request_type)} — ${esc(x.title)}</b><p>العميل: ${esc(x.customer_name)} | ${esc(x.phone)}</p><p>الموعد: ${esc(x.scheduled_at)}</p><p>${esc(x.notes||'لا توجد ملاحظات')}</p><select id="owner-appointment-${x.id}"><option value="pending" ${x.status==='pending'?'selected':''}>معلق</option><option value="accepted" ${x.status==='accepted'?'selected':''}>مقبول</option><option value="rejected" ${x.status==='rejected'?'selected':''}>مرفوض</option><option value="completed" ${x.status==='completed'?'selected':''}>مكتمل</option></select><button onclick="updateSupportAppointment(${x.id},${organizationId})">حفظ حالة الموعد</button></div>`).join('')||'<p class="muted">لا توجد مواعيد لهذه المؤسسة.</p>'}
async function updateSupportAppointment(id,organizationId){let status=document.getElementById('owner-appointment-'+id).value;if(!confirm('تأكيد تغيير حالة الموعد؟'))return;let r=await fetch('/owner/api/appointments/'+id,{method:'PUT',headers:hdr(),body:JSON.stringify({status})}),d=await r.json();alert(r.ok?'تم تحديث حالة الموعد ✓':(d.error||'تعذر تحديث الموعد'));if(r.ok)loadSupportAppointments(organizationId)}
...
<script>const headers=()=>({'Content-Type':'application/json','X-Owner-Key':document.getElementById('key').value.trim()});async function login(){let r=await fetch('/owner/api/codes',{headers:headers()});let d=await r.json();document.getElementById('status').textContent=r.ok?'تم الدخول بنجاح ✓':(d.error||'تعذر الدخول');if(r.ok)render(d)}async function createCode(){let r=await fetch('/owner/api/codes',{method:'POST',headers:headers(),body:JSON.stringify({recipientName:document.getElementById('recipient').value,customCode:document.getElementById('customCode').value,discountPercent:+document.getElementById('discount').value,validityDays:+document.getElementById('validityDays').value,maxUses:+document.getElementById('uses').value})});let d=await r.json();document.getElementById('result').textContent=r.ok?(d.renewed?'تم تجديد الكود: ':'تم إنشاء الكود: ')+d.code+'\\nالحملة: '+(d.recipientName||'غير محدد')+'\\nالخصم: '+d.discountPercent+'%':(d.error||'تعذر إنشاء الكود');if(r.ok)loadCodes()}async function loadCodes(){let r=await fetch('/owner/api/codes',{headers:headers()});let d=await r.json();if(r.ok)render(d);else document.getElementById('codes').textContent=d.error||'تعذر تحميل الأكواد'}function render(data){let box=document.getElementById('codes');box.innerHTML=data.length?data.map(c=>`<div class="card code"><b>${c.code_prefix}...</b><p><b>الحملة أو المعلن:</b> ${c.recipient_name||'غير محدد'}</p><p><b>نسبة الخصم:</b> ${c.discount_percent||0}%</p><p><b>مرات الاستخدام:</b> ${c.used_count} من ${c.max_uses}</p><p><b>تاريخ انتهاء الكود:</b> ${c.expires_at||'بدون انتهاء'}</p><p><b>النوع:</b> ${c.code_kind==='discount'?'كود خصم':'كود قديم'}</p></div>`).join(''):'لا توجد أكواد بعد'}</script></body></html>"""
        ...
        if path.startswith("/owner/api/organizations/") and path.endswith("/appointments") and method == "GET":
            ...
            self._send(200, [dict(row) for row in rows])
            return
        if path.startswith("/owner/api/appointments/") and method == "PUT":
            self._owner()
            appointment_id = int(path.rsplit("/", 1)[1])
            status = str(self._body().get("status", "")).strip()
            if status not in ("pending", "accepted", "rejected", "completed"):
                raise ApiError(400, "حالة الموعد غير صحيحة")
            with db() as connection:
                appointment = connection.execute("SELECT organization_id,request_type,customer_name FROM appointment_requests WHERE id=?", (appointment_id,)).fetchone()
                if appointment is None:
                    raise ApiError(404, "الموعد غير موجود")
                connection.execute("UPDATE appointment_requests SET status=?,updated_at=? WHERE id=?", (status, now(), appointment_id))
                audit_log(connection, appointment["organization_id"], None, "owner_appointment_status", f"غيّر دعم خدووم حالة {appointment['request_type']} للعميل {appointment['customer_name']} إلى {status}", "appointment", appointment_id)
                connection.commit()
