...
function renderDevices(active,blocked){let groups=new Map();let group=x=>{let key=x.organization_id||x.organization_name;if(!groups.has(key))groups.set(key,{name:x.organization_name,active:[],blocked:[]});return groups.get(key)};active.forEach(x=>group(x).active.push(x));blocked.forEach(x=>group(x).blocked.push(x));document.getElementById('devices').innerHTML=[...groups.values()].map(g=>`<details class="org-devices"><summary>${esc(g.name)} <span class="status-chip">${g.active.length} جهاز نشط</span> ${g.blocked.length?`<span class="status-chip danger">${g.blocked.length} محظور</span>`:''}</summary><div>${g.active.map(x=>`<div class="device-user"><b>${esc(x.user_name)}</b><p>اسم المستخدم: ${esc(x.username)} | <span class="status-chip ${x.user_active?'ok':'danger'}">${x.user_active?'الحساب نشط':'الحساب مجمّد'}</span></p><p>الجهاز: ${esc(x.device_name||'جهاز غير معروف')}</p><p>آخر استخدام: ${esc(x.last_seen_at||x.created_at)} <span class="status-chip ${x.trusted?'ok':'warn'}">${x.trusted?'موثق':'غير موثق'}</span></p><div class="device-actions"><button onclick="deviceTrust('${x.id}',${!x.trusted})">${x.trusted?'إلغاء التوثيق':'توثيق'}</button><button onclick="disconnectDevice('${x.id}')">فصل</button><button onclick="blockDevice('${x.id}')">حظر</button></div></div>`).join('')}${g.blocked.map(x=>`<div class="device-user"><b>${esc(x.device_name||'جهاز غير معروف')}</b><p><span class="status-chip danger">محظور</span> منذ ${esc(x.blocked_at)}</p><div class="device-actions"><button onclick="unblockDevice(${x.organization_id},'${x.device_id}')">فك الحظر</button></div></div>`).join('')}</div></details>`).join('')||'<p>لا توجد أجهزة أو جلسات.</p>'}
