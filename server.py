...
function renderBackup(x){document.getElementById('backupStatus').innerHTML=`<p><b>قاعدة البيانات:</b> ${esc(x.provider||'غير معروفة')}</p><p><b>الاتصال:</b> <span class="${x.connected?'ok':'danger'}">${x.connected?'سليم ✓':'متعطل'}</span></p><p><b>آخر فحص:</b> ${esc(x.checkedAt||'غير متاح')}</p><p><b>نافذة الاستعادة:</b> ${x.recoveryHours?esc(x.recoveryHours)+' ساعات':'غير متاحة'}</p><p><b>Snapshot يدوي:</b> ${x.manualSnapshots?'متاح':'غير متاح في الخطة الحالية'}</p><p><b>نسخ مجدولة:</b> ${x.scheduledBackups?'مفعلة':'غير مفعلة'}</p><p class="muted">${esc(x.note||'')}</p>`}function renderEmergency(items){document.getElementById('emergencyStatus').innerHTML=items.map(x=>`<p><b>${x.service==='chat'?'شات العملاء':x.service==='appointments'?'المواعيد':'مساعد المؤسسة'}:</b> <span class="${x.active?'danger':'ok'}">${x.active?'متوقفة حتى '+esc(x.until):'تعمل'}</span></p>`).join('')}
