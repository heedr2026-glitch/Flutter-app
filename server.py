...
<section id="devicesPanel" class="security-panel"><div class="card"><h2>الأجهزة والجلسات</h2><p class="muted">افتح المؤسسة لمشاهدة المستخدمين والأجهزة وحالة كل جهاز وإجراءاته.</p><div id="devices"></div></div></section>
...
function renderDevices(active,blocked){let groups=new Map();let group=x=>{let key=x.organization_id||x.organization_name;if(!groups.has(key))groups.set(key,{name:x.organization_name,active:[],blocked:[]});return groups.get(key)};active.forEach(x=>group(x).active.push(x));blocked.forEach(x=>group(x).blocked.push(x));document.getElementById('devices').innerHTML=[...groups.values()].map(g=>{let users=new Map();g.active.forEach(x=>{let key=x.username||x.user_name;if(!users.has(key))users.set(key,{name:x.user_name,username:x.username,active:x.user_active,devices:[]});users.get(key).devices.push(x)});let ids=g.active.map(x=>x.id);return `<details class="org-devices"><summary>${esc(g.name)} <span class="status-chip">${g.active.length} جلسة مسجلة</span> ${g.blocked.length?`<span class="status-chip danger">${g.blocked.length} جهاز محظور</span>`:''}</summary><div class="device-user"><b>إجراءات جميع أجهزة المؤسسة</b><p class="muted">تطبق على الأجهزة الظاهرة لهذه المؤسسة فقط.</p><div class="device-actions"><button onclick='organizationDevicesAction(${JSON.stringify(ids)},"trust")'>توثيق الكل</button><button onclick='organizationDevicesAction(${JSON.stringify(ids)},"disconnect")'>فصل الكل</button><button onclick='organizationDevicesAction(${JSON.stringify(ids)},"block")'>حظر الكل</button></div></div><h3 style="padding:0 12px">المستخدمون</h3>${[...users.values()].map(u=>`<details class="device-user"><summary><b>${esc(u.name)}</b> — ${esc(u.username)} <span class="status-chip ${u.active?'ok':'danger'}">${u.active?'الحساب نشط':'الحساب مجمّد'}</span></summary>${u.devices.map(x=>`<div class="device-user"><b>${esc(x.device_name||'جهاز غير معروف')}</b><p><span class="status-chip ${recentlySeen(x.last_seen_at)?'ok':'warn'}">${recentlySeen(x.last_seen_at)?'متصل الآن':'غير متصل حاليًا'}</span> <span class="status-chip ${x.trusted?'ok':'warn'}">${x.trusted?'موثق':'غير موثق'}</span></p><p>آخر استخدام: ${esc(x.last_seen_at||x.created_at)}</p><div class="device-actions"><button onclick="deviceTrust('${x.id}',${!x.trusted})">${x.trusted?'إلغاء التوثيق':'توثيق'}</button><button onclick="disconnectDevice('${x.id}')">فصل</button><button onclick="blockDevice('${x.id}')">حظر</button></div></div>`).join('')}</details>`).join('')}${g.blocked.length?`<h3 style="padding:0 12px">الأجهزة المحظورة</h3>${g.blocked.map(x=>`<div class="device-user"><b>${esc(x.device_name||'جهاز غير معروف')}</b><p><span class="status-chip danger">محظور</span> منذ ${esc(x.blocked_at)}</p><div class="device-actions"><button onclick="unblockDevice(${x.organization_id},'${x.device_id}')">فك الحظر</button></div></div>`).join('')}`:''}</div></details>`}).join('')||'<p>لا توجد أجهزة أو جلسات.</p>'}
async function organizationDevicesAction(ids,action){if(!ids.length){alert('لا توجد أجهزة مسجلة لهذه المؤسسة');return}let label=action==='trust'?'توثيق جميع الأجهزة':action==='disconnect'?'فصل جميع الأجهزة':'حظر جميع الأجهزة';if(!confirm('تأكيد '+label+'؟'))return;let requests=ids.map(id=>fetch('/owner/api/security/devices/'+id+(action==='block'?'/block':''),{method:action==='trust'?'PUT':action==='disconnect'?'DELETE':'POST',headers:hdr(),body:action==='trust'?JSON.stringify({trusted:true}):null}));let responses=await Promise.all(requests);alert(responses.every(r=>r.ok)?'تم تنفيذ العملية على أجهزة المؤسسة ✓':'تعذر تنفيذ العملية على بعض الأجهزة');loadSecurity()}async function accountAction(id,active){if(!confirm(active?'تأكيد إعادة تفعيل الحساب؟':'تأكيد تجميد الحساب؟'))return;await act('/owner/api/security/accounts/'+id,'PUT',{active})}
