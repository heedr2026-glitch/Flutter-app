...
<section id="subscriptionSecurityPanel" class="security-panel"><div class="card"><h2>طلبات الاشتراك والتحويلات</h2><p class="muted">راجع اسم المحوّل والوصل قبل تفعيل الباقة.</p><button onclick="loadSecuritySubscriptionRequests()">تحديث الطلبات</button><div id="securitySubscriptionRequests"></div></div></section>
...
let supportRefreshTimer=setInterval(()=>{if(document.getElementById('dashboard').style.display!=='none')checkNewSupportTickets()},15000);let supportInitialized=false;function notifySupportTicket(x){if(!('Notification' in window))return;const show=()=>{const n=new Notification('طلب دعم جديد من مشترك 🛠️',{body:(x.organization_name||'مؤسسة')+' — '+(x.category||'مشكلة فنية'),tag:'khdoom-support-'+x.id});n.onclick=()=>{window.focus();openSupportTicket(x.id);n.close()}};if(Notification.permission==='granted')show();else if(Notification.permission!=='denied')Notification.requestPermission().then(p=>{if(p==='granted')show()})}async function openSupportTicket(id){showSecurityPanel('emergencyPanel');await loadSupportTickets();const item=document.getElementById('support-ticket-'+id);if(item)item.scrollIntoView({behavior:'smooth',block:'center'});}async function checkNewSupportTickets(){try{const r=await fetch('/owner/api/support-tickets',{headers:hdr(),cache:'no-store'}),items=await r.json();if(!r.ok||!Array.isArray(items))return;const newest=items.filter(x=>x.status==='open').sort((a,b)=>Number(b.id)-Number(a.id))[0],last=Number(localStorage.getItem('khdoom_last_support_ticket_id')||0);if(newest&&supportInitialized&&Number(newest.id)>last){notifySupportTicket(newest);localStorage.setItem('khdoom_last_support_ticket_id',String(newest.id))}else if(!supportInitialized&&newest){localStorage.setItem('khdoom_last_support_ticket_id',String(newest.id))}supportInitialized=true;loadSupportTickets()}catch(e){}}
...
async function act(url,method,body){let r=await fetch(url,{method,headers:hdr(),body:body?JSON.stringify(body):null}),d=await r.json();alert(r.ok?'تم تنفيذ العملية ✓':(d.error||'تعذر تنفيذ العملية'));if(r.ok)loadSecurity()}
                    ...
                    '</style>',
                    '</style><style>body{background:#061a3c;color:#fff}.card{background:#12356f;border-color:#5aa9ee;box-shadow:0 8px 24px #02061755}.metric{background:#174887;color:#fff}.metric strong{color:#fff}.event{background:#1a4c8f;border-right-color:#7dd3fc}.muted{color:#e0f2fe}input,select,button{background:#0b2858;color:#fff;border-color:#5aa9ee}button{background:#168bd1}.category-button,.org-devices summary{background:#1b579c;color:#fff;border-color:#7dd3fc}.service-row,.device-user{background:#19427e;border-color:#5aa9ee}</style>',
                    1,
                )
            )
            return
        ...
        if path == "/owner/api/support-tickets" and method == "GET":
            self._owner()
            with db() as connection:
                # Older production databases may not have the support tables
                # yet. Initialize only the additive support schema before
                # reading it so opening the owner support page cannot become a
                # generic 500 error.
                owner_admin.ensure_owner_tables(
                    connection,
                    __import__('sys').modules[__name__],
...
<script>const headers=()=>({'Content-Type':'application/json','X-Owner-Key':document.getElementById('key').value.trim()});async function login(){let r=await fetch('/owner/api/codes',{headers:headers()});let d=await r.json();document.getElementById('status').textContent=r.ok?'تم الدخول بنجاح ✓':(d.error||'تعذر الدخول');if(r.ok)render(d)}async function createCode(){let r=await fetch('/owner/api/codes',{method:'POST',headers:headers(),body:JSON.stringify({recipientName:document.getElementById('recipient').value,customCode:document.getElementById('customCode').value,discountPercent:+document.getElementById('discount').value,validityDays:+document.getElementById('validityDays').value,maxUses:+document.getElementById('uses').value})});let d=await r.json();document.getElementById('result').textContent=r.ok?(d.renewed?'تم تجديد الكود: ':'تم إنشاء الكود: ')+d.code+'\\nالحملة: '+(d.recipientName||'غير محدد')+'\\nالخصم: '+d.discountPercent+'%':(d.error||'تعذر إنشاء الكود');if(r.ok)loadCodes()}async function loadCodes(){let r=await fetch('/owner/api/codes',{headers:headers()});let d=await r.json();if(r.ok)render(d);else document.getElementById('codes').textContent=d.error||'تعذر تحميل الأكواد'}function render(data){let box=document.getElementById('codes');box.innerHTML=data.length?data.map(c=>`<div class="card code"><b>${c.code_prefix}...</b><p><b>الحملة أو المعلن:</b> ${c.recipient_name||'غير محدد'}</p><p><b>نسبة الخصم:</b> ${c.discount_percent||0}%</p><p><b>مرات الاستخدام:</b> ${c.used_count} من ${c.max_uses}</p><p><b>تاريخ انتهاء الكود:</b> ${c.expires_at||'بدون انتهاء'}</p><p><b>النوع:</b> ${c.code_kind==='discount'?'كود خصم':'كود قديم'}</p></div>`).join(''):'لا توجد أكواد بعد'}</script></body></html>"""
        ...
        with db() as connection:
            ...
            if path.startswith("/api/ads/") and method == "DELETE":
                ...
                if ad is None:
                    raise ApiError(404, "الإعلان غير موجود")
                if ad["approved"] or ad["active"]:
                    raise ApiError(400, "يمكن حذف الإعلانات المرفوضة فقط")
                connection.execute("UPDATE advertisements SET active=0,deleted=1 WHERE id=? AND organization_id=?", (ad_id, organization_id))
                audit_log(connection, organization_id, user["id"], "advertisement_deleted", f"حذف الإعلان المرفوض رقم {ad_id}", "advertisement", str(ad_id))
                connection.commit()
                self._send(200, {"deleted": True})
                return
            ...
            if path == "/api/subscription-requests" and method == "POST":
                ...
                if not transfer_receipt:
                    raise ApiError(400, "إرفاق صورة وصل التحويل مطلوب")
